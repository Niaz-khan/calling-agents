...
from apps.conversations.call_intelligence import finalize_call
...
def get_conversation_by_stream_token(stream_token: str) -> Conversation | None:
    """Resolve a media-stream websocket session by its per-call secret token.

    The token is generated when the call is created and embedded in the TwiML
    ``<Stream>`` url, so a websocket client that does not own the call cannot
    ...
    phone_call = (
        PhoneCall.objects.filter(stream_token=stream_token)
        .select_related(
            "conversation",
            "conversation__agent",
            "conversation__organization",
            "conversation__phone_call",
        )
        .first()
    )
...
def get_twilio_stream_url(conversation: Conversation) -> str:
    """Build the wss:// URL Twilio connects for a call's media stream.

    The url embeds the per-call stream token so the websocket consumer can
    resolve (and authenticate) the call without trusting any client field.
    """
    base = settings.PUBLIC_BASE_URL.rstrip("/")
    scheme = "wss" if base.startswith("https://") else "ws"
    host = base.split("://", 1)[-1]
    token = conversation.phone_call.stream_token
    return f"{scheme}://{host}/telephony/twilio/media?token={token}"
