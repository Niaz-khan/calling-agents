...
def build_stream_twiml(stream_url: str) -> str:
    """TwiML that starts a bidirectional G.711 media stream.

    ``<Connect><Stream>`` wires the call's audio to the given websocket; while
    the websocket is open the caller's audio flows to us and anything we send
    back is played to the caller. When the websocket closes, the call moves to
    the next verb (none here), so the call ends cleanly.
    """
    return (
        '<?xml version="1.0" encoding="UTF-8"?>'
        '<Response><Connect><Stream url="' + escape(stream_url) + '"/></Connect></Response>'
    )
