...
class TestAgentChat:
    def test_chat_returns_response(self, tenant, monkeypatch):
        ...
        monkeypatch.setattr(
            "apps.ai.agent.generate_response",
    ...
    def test_chat_llm_unavailable_returns_503(self, tenant, monkeypatch):
        ...
        monkeypatch.setattr("apps.ai.agent.generate_response", boom)
...
class TestPublicChat:
    ...
    def test_chat_creates_website_conversation(self, tenant, api_client, monkeypatch):
        ...
        _, org, _ = tenant
        deployment = self._deployment(org)
        monkeypatch.setattr("apps.ai.agent.generate_response", lambda messages, tools=None: {
    ...
    def test_llm_error_returns_503(self, tenant, api_client, monkeypatch):
        ...
        monkeypatch.setattr("apps.agents.public.run_agent_turn", boom)
    ...
    def test_throttling_returns_429(self, tenant, api_client, monkeypatch):
        import apps.agents.public as public_module

        _, org, _ = tenant
        deployment = self._deployment(org)
        monkeypatch.setattr("apps.agents.public.THROTTLE_LIMIT_PER_WINDOW", 3)
        monkeypatch.setattr("apps.agents.public.THROTTLE_WINDOW_SECONDS", 3600)
        monkeypatch.setattr("apps.ai.agent.generate_response", lambda messages, tools=None: {
            "content": "ok", "tool_calls": []
        })
...
class TestWidgetConfig:
    ...
    def test_config_online_matches_business_presence(self, tenant, api_client):
        from apps.tenancy.services import is_business_open
